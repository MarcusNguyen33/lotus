...
export async function POST(r:Request) {
  ...
  try {
    ...
    if(d.kind==='order') {
      ...
      if(current.payment_status==='paid'&&Number(current.shipping_cost)!==d.shipping_cost)return Response.json({error:'Không thể đổi phí của đơn đã thanh toán.'},{status:409});
      const shipping=Math.round(d.shipping_cost*100)/100;
      await supabaseOne('orders',{method:'PATCH',body:JSON.stringify({status:d.status,shipment:d.shipment,shipping_cost:shipping,grand_total:Math.round((Number(current.subtotal)+shipping)*100)/100})},filter({id:`eq.${d.id}`}));
    } else {
